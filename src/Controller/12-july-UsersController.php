...
//                 mens_brands style_sphere_selections_v2
            ...
            if (@$data['first_time_fix'] == 'first_time_fix') {
                ...
                if (isset($data['kid_id']) && $data['kid_id']) {
                    $getdata = $this->LetsPlanYourFirstFix->find('all')->where(['LetsPlanYourFirstFix.kid_id' => @$data['kid_id'], 'LetsPlanYourFirstFix.user_id' => $this->Auth->user('id')])->first();
                    if (@$getdata->kid_id) {
                        $data['id'] = $getdata->id;
                    } else {
                        $data['id'] = '';
                ...
                if ($exitdata >= 1) {
                    if (isset($data['kid_id']) && $data['kid_id']) {
                        $this->LetsPlanYourFirstFix->updateAll(['try_new_items_with_scheduled_fixes' => $try_new_items_with_scheduled_fixes, 'how_often_would_you_lik_fixes' => $how_often_would_you_lik_fixes], ['kid_id' => $data['kid_id']]);
                    } else {
...
//                    pj($LetsPlanYourFirstFix);exit;
            ...
            if (@$data['deliverAddress'] == 'deliverAddress') {
                ...
                if ($data['id']) {
                    ...
                    $countS = $this->ShippingAddress->find('all')->where(['ShippingAddress.user_id' => $this->Auth->user('id'), 'ShippingAddress.default_set' => 1])->count();
...
//                pj($table);exit;
            ...
            if (@$this->request->session()->read('PROFILE') == 'KIDS') {
                $this->KidsDetails->updateAll(['is_redirect' => 4], ['id' => @$this->request->session()->read('KID_ID')]);
            } else {
                $this->Users->updateAll(['is_redirect' => 4], ['id' => $this->Auth->user('id')]);
            }
    ...
    public function customerOrderReview() {
        ...
        if ($this->request->is('post')) {
            ...
            if ($paymentId) {
                ...
                if (@$exCountKeeps != 0) {
                    if (@$productCount == @$exCountKeeps) {
                        $percentage = 25;
                        $discount = ($percentage / 100) * $grand_price;
                    } else {
                        @$purchaseCredit = 19;
                    }
                }
            ...
            $grand_price1 = $grand_price - @$discount;

            $sales_tax = ( $grand_price1 * 8.25) / 100;
            $total = $sales_tax + $grand_price1 - @$purchaseCredit;
            $getWalltesCredit = $this->Wallets->find('all')->where(['Wallets.user_id' => $this->request->session()->read('Auth.User.id'), 'Wallets.type' => 2, 'Wallets.applay_status' => 0])->sumOf('balance');

            $getWalltesDebit = $this->Wallets->find('all')->where(['Wallets.user_id' => $this->request->session()->read('Auth.User.id'), 'Wallets.type' => 1, 'Wallets.applay_status' => 0])->sumOf('balance');

            $walletBalace = $getWalltesCredit - $getWalltesDebit;
            if ($walletBalace >= $total) {
                $pendingBlace = 0;
            } else {
                $pendingBlace = $total - $walletBalace;
            }
            ...
            $table['wallet_balance'] = $data['walletBalance'];
            $table['promo_balance'] = $data['applayBalance'];
            ...
            if ($message['status'] == '1') {
                ...
                if (@$data['walletCheck'] == '1') {
                    if ($walletBalace >= $total) {
                        $remaining = $walletBalace - $total;
                        $walletsEnRE = $this->Wallets->newEntity();
                        $walletsEnRE->user_id = $this->request->session()->read('Auth.User.id');
                        $walletsEnRE->type = 1; //debited
                        $walletsEnRE->balance = $total;
                        $walletsEnRE->created = date('Y-m-d h:i:s');
                        ...
                        $walletsEnRE->user_id = $this->request->session()->read('Auth.User.id');
                        $walletsEnRE->type = 1;
                        $walletsEnRE->balance = $walletBalace;
                        $walletsEnRE->created = date('Y-m-d h:i:s');
                        $walletsEnRE->applay_status = 0;
                        $this->Wallets->save($walletsEnRE);
                    }
                ...
                foreach ($prData as $dataMail) {
                    ...
                    if ($dataMail->keep_status == 3) {
                        ...
                        $keep = 'Exchange';
                        $this->Products->updateAll(['is_complete' => '0'], ['id' => $dataMail->id]);
                ...
                if ($paymentId) {
                    ...
                    // $exCountexchange = $this->Products->find('all')->where(['Products.payment_id' => $paymentId, 'Products.keep_status IN' => [1, 2, 3], 'is_complete' => 0])->Count();
        ...
        $this->set(compact('walletBalace', 'prData', 'userData', 'cname', 'allKeepsProducts', 'percentage', 'purchaseCredit'));
    }
    ...
    public function notYetShipped() {
        ...
        $getProductDetails = $this->Products->find('all')->where(['payment_id' => $paymentIdown, 'is_altnative_product' => 0, 'keep_status' => 2])->count();
...
// }
    ...
    public function getBillingAddressDetails() {
        $this->viewBuilder()->setLayout('ajax');
        $billingAddress = $this->ShippingAddress->find('all')->where(['ShippingAddress.user_id' => $this->Auth->user('id'), 'ShippingAddress.default_set' => 1])->first();
        ...
        if ($this->request->is('post')) {
            ...
            if ($data['promocode']) {
                ...
                if ((@$checkData->id == '') && ($checkDataGift == '')) {
                    ...
                    $checkUserApplyDataGift = $this->UserUsesGiftcode->find('all')->where(['UserUsesGiftcode.giftcode' => @$checkDataGift->giftcode, 'UserUsesGiftcode.user_id' => $this->Auth->user('id')])->first();


                    if (@$checkUserApplyData->id == '' && @$checkUserApplyDataGift == '') {
                        if (@$checkData->price != '') {
        ...
        if ($this->request->is('post')) {
            ...
            if ($data['promocode']) {
                ...
                if ((@$checkData->id == '') && (@$checkDataGift == '')) {
                    ...
                    if (@$checkDataGift->giftcode != '') {

                        $checkUserApplyDataGift = $this->UserUsesGiftcode->find('all')->where(['UserUsesGiftcode.giftcode' => @$checkDataGift->promocode, 'UserUsesGiftcode.user_id' => $this->Auth->user('id')])->first();


                        if (@$checkUserApplyDataGift->id == '') {
                            $UserUsesPromocode = $this->UserUsesGiftcode->newEntity();
                            $UserUsesPromocode->user_id = $this->Auth->user('id');
                            $UserUsesPromocode->giftcode = $checkDataGift->giftcode;
                            $UserUsesPromocode->apply_dt = date('Y-m-d H:i:s');
                            $UserUsesPromocode->price = $checkDataGift->price;
                            $this->UserUsesGiftcode->save($UserUsesPromocode);

                            $wallets = $this->Wallets->newEntity();
                            $wallets->user_id = $this->Auth->user('id');
                            $wallets->type = 2;
                            $wallets->balance = $checkDataGift->price;
                            $wallets->created = date('Y-m-d h:i:s');
                            $wallets->applay_status = 0;
                            $wallet = $this->Wallets->save($wallets);
                            $price = number_format((float) $checkDataGift->price, 2, '.', '');

                            if ($checkDataGift->price > $data['totalPrice']) {
                                $pendingBlc = $checkDataGift->price - $data['totalPrice'];
                                echo json_encode(['status' => 'successgift', 'msg' => 'Your  balance  -$' . $price . ' <br> $' . $checkDataGift->price . '-' . $data['totalPrice'] . '= $' . $pendingBlc . ' <br>remaining balance is your wallet save', 'price' => $price, 'walletId' => $wallet->id]);
                            } else {
                                echo json_encode(['status' => 'successgift', 'msg' => 'Your  balance  -$' . $price, 'price' => $price, 'walletId' => $wallet->id]);
                            }
                        }
                    }
                    ...
                    if (@$checkData->id != '') {
                        ...
                        if (@$checkUserApplyData->id == '') {
                            ...
                            if (@$checkData->price > @$data['totalPrice']) {
                                $pendingBlc = $checkData->price - $data['totalPrice'];
                                echo json_encode(['status' => 'success', 'msg' => 'Your  balance  -$' . $price . ' <br> $' . $checkData->price . '-' . $data['totalPrice'] . '= $' . $pendingBlc . ' <br>remaining balance is your wallet save', 'price' => $price, 'walletId' => $wallet->id]);
                            } else {
                                echo json_encode(['status' => 'success', 'msg' => 'Your  balance  -$' . $price, 'price' => $price, 'walletId' => $wallet->id]);
                            }
                        }
                    }
                }
